...
parseChordGrid(source: string): ChordGrid {
    ...
    for (const line of lines) {
        ...
        console.log("Tokens for line:", trimmed, tokens); // DEBUG
        ...
        for (const token of tokens) {
            if (token.type === 'repeatStart') {
                isRepeatStart = true;
                console.log("Found repeatStart"); // DEBUG
            } else if (token.type === 'repeatEnd') {
                isRepeatEnd = true;
                console.log("Found repeatEnd"); // DEBUG
                ...
                if (currentMeasure.chords.length > 0) {
                    ...
                    console.log("Pushed measure with repeatEnd:", isRepeatEnd); // DEBUG
                    // Start a new measure
                    currentMeasure = { 
                        chords: [], 
                        hasBarLine: false,
        ...
        if (currentMeasure.chords.length > 0) {
            currentMeasure.isRepeatEnd = isRepeatEnd;
            measures.push(currentMeasure);
            console.log("Pushed last measure with repeatEnd:", isRepeatEnd); // DEBUG
        }

        // Apply repeat start to the FIRST measure if we have one
        if (measures.length > 0 && isRepeatStart) {
            measures[0].isRepeatStart = true;
            console.log("Applied repeatStart to first measure"); // DEBUG
        }
    ...
    console.log("Final measures:", measures); // DEBUG
...
tokenizeLine(line: string): Array<{type: string, chord?: string, beats?: Beat[], rhythm?: string}> {
    ...
    while (remaining.length > 0) {
        ...
        // Check for repeat markers - CORRECTION : vérifier d'abord :||
    ...
    for (let i = 0; i < notePositions.length - 1; i++) {
      ...
      if (currentNote.tied && 
        ...
        ties.push({
          ...
          startY: currentNote.y - 8, // LOWER position for ties
          endX: nextNote.x,
          endY: nextNote.y - 8, // LOWER position for ties
          type: 'internal'
        });
      }
    }
  ...
  drawTie(svg: SVGElement, tie: TiePosition) {
    ...
    const controlY = tie.startY - 5; // LESS curve height
...
/**
 * Draw a double bar line with proper spacing
 */
drawDoubleBar(svg: SVGElement, x: number, y: number, height: number) {
    ...
    // CORRECTION : Ajouter de l'espace entre les deux barres
    const bar2 = document.createElementNS('http://www.w3.org/2000/svg', 'line');
    bar2.setAttribute('x1', (x + 6).toString()); // +6px d'espace
...
/**
 * Draw a bar with repeat symbols - DOUBLE barre avec points
 */
drawBarWithRepeat(svg: SVGElement, x: number, y: number, height: number, isStart: boolean) {
    // CORRECTION : Utiliser double barre avec espace
    this.drawDoubleBar(svg, x, y, height);
    
    // CORRECTION : Points à DROITE pour début, à GAUCHE pour fin
    const dotOffset = isStart ? 12 : -12; // Ajusté pour la double barre
  ...
  drawRhythm(svg: SVGElement, chordData: ChordInMeasure, x: number, staffLineY: number, width: number, measureIndex: number, chordIndex: number, notePositions: {x: number, y: number, measureIndex: number, chordIndex: number, beatIndex: number, noteIndex: number, tied: boolean}[]): number | null {
    ...
    beats.forEach((beat, beatIndex) => {
      // CORRECTION : Dessiner le beat en commençant au début du segment (currentX)
  ...
  drawBeat(svg: SVGElement, beat: Beat, x: number, staffLineY: number, width: number, measureIndex: number, chordIndex: number, beatIndex: number, notePositions: {x: number, y: number, measureIndex: number, chordIndex: number, beatIndex: number, noteIndex: number, tied: boolean}[]): number | null {
    if (beat.notes.length === 1) {
      ...
      // CORRECTION : Positionner la note au début du segment + petit offset
      ...
      // CORRECTION : Même principe pour les groupes de notes
...
drawSingleNote(svg: SVGElement, nv: NoteValue, x: number, staffLineY: number, width: number): number {
  // CORRECTION : Utiliser la position x directement (déjà ajustée)
  const centerX = x;

  // Draw appropriate note head based on duration
  if (nv.value === 1) {
    // Whole note - white diamond note head (no stem)
    this.drawDiamondNoteHead(svg, centerX, staffLineY, true); // true = white/hollow
  } else if (nv.value === 2) {
    // Half note - white diamond note head with stem
    this.drawDiamondNoteHead(svg, centerX, staffLineY, true);
    this.drawStem(svg, centerX, staffLineY, 25);
  } else {
    // Quarter note and shorter - slash note head
    this.drawSlash(svg, centerX, staffLineY);
    
    // Draw stem for short notes
    this.drawStem(svg, centerX, staffLineY, 25);
...
drawDiamondNoteHead(svg: SVGElement, x: number, y: number, hollow: boolean) {
  ...
  const points = [
    [x, y - diamondSize],           // Top
    [x + diamondSize, y],           // Right
    [x, y + diamondSize],           // Bottom
    [x - diamondSize, y]            // Left
  ];
  
  diamond.setAttribute('points', points.map(p => `${p[0]},${p[1]}`).join(' '));
  diamond.setAttribute('fill', hollow ? 'white' : 'black'); // White for hollow, black for filled
...
drawNoteGroup(svg: SVGElement, notesValues: NoteValue[], x: number, staffLineY: number, width: number): number | null {
  ...
  for (let i = 0; i < noteCount; i++) {
    ...
    // Draw appropriate note head based on duration
    if (nv.value === 1) {
      // Whole note - white diamond note head (no stem in groups)
      this.drawDiamondNoteHead(svg, centerX, staffLineY, true);
  ...
  // Only draw beams for notes that can be beamed (8th notes and shorter)
