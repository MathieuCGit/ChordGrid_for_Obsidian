...
export class RestRenderer {
   ...
   */
  drawRest(svg: SVGElement, note: NoteElement, x: number, y: number, stemsDirection: 'up' | 'down' = 'up') {
    if (note.value === 1) {
  ...
  private drawQuarterRest(svg: SVGElement, x: number, y: number, dotted: boolean, stemsDirection: 'up' | 'down' = 'up') {
    ...
    const Y_OFFSET = stemsDirection === 'up' ? 12 : -8; // Lift up 12px in stem-up, lower 8px in stem-down
  ...
  private drawEighthRest(svg: SVGElement, x: number, y: number, dotted: boolean, stemsDirection: 'up' | 'down' = 'up') {
  ...
  private drawSixteenthRest(svg: SVGElement, x: number, y: number, dotted: boolean, stemsDirection: 'up' | 'down' = 'up') {
  ...
  private drawThirtySecondRest(svg: SVGElement, x: number, y: number, dotted: boolean, stemsDirection: 'up' | 'down' = 'up') {
